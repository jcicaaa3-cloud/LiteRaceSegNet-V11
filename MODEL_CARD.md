...
SegFormer-03 is kept as a separate Transformer baseline. It is not merged into LiteRaceSegNet and is not presented as the proposed model.
...
Do not compare CPU latency and GPU latency as if they were the same condition. Compare LiteRaceSegNet vs SegFormer-03 within the same device condition.
...
LiteRaceSegNet-related code, documentation, diagrams, experiment notes, configuration files, and scripts in this repository are public only for portfolio viewing and academic demonstration. Copying, redistribution, modification, publication, derivative use, commercial use, or sublicensing is not permitted without prior permission from the author.

Datasets, pretrained weights, Hugging Face files, third-party libraries, and referenced model implementations remain under their own licenses and terms.