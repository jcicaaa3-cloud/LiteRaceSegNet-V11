...
echo It runs the same LiteRaceSegNet vs SegFormer-03 comparison twice:
...
if not exist "seg\transformer_03\checkpoints\segformer_03_best.pth" (
  echo [ERROR] Fine-tuned SegFormer-03 checkpoint not found.
  echo Run 03B_TRAIN_SEGFORMER_03_ONLY.bat first.
  pause
  exit /b 1
)
