...
def _tradeoff_note(rows: List[Dict[str, str]]) -> str:
  ...
  for device, device_rows in groups.items():
    ...
    if _is_number(lite_size) and _is_number(seg_size):
      ...
      if ratio > 1:
        notes.append(f"- FP32 파라미터 크기: SegFormer-03/LiteRaceSegNet = {ratio:.2f}x")
      else:
        notes.append("- FP32 파라미터 크기에서는 LiteRaceSegNet이 SegFormer-03보다 작게 나오지 않았습니다. config와 모델 로딩 상태를 확인하세요.")
    if _is_number(lite_lat) and _is_number(seg_lat):
      ratio = float(seg_lat) / max(float(lite_lat), 1e-12)
      if ratio > 1:
        notes.append(f"- {device_label} 평균 latency: SegFormer-03/LiteRaceSegNet = {ratio:.2f}x. LiteRaceSegNet이 더 빠릅니다.")
      else:
        notes.append(f"- {device_label} 평균 latency: LiteRaceSegNet이 SegFormer-03보다 빠르게 나오지 않았습니다. batch size, image size, device 상태를 같이 확인하세요.")
    if device == "cuda" and _is_number(lite_mem) and _is_number(seg_mem):
      ratio = float(seg_mem) / max(float(lite_mem), 1e-12)
      notes.append(f"- CUDA peak memory: SegFormer-03/LiteRaceSegNet = {ratio:.2f}x")
    if _is_number(lite_miou) and _is_number(seg_miou):
      diff = float(lite_miou) - float(seg_miou)
      notes.append(f"- mIoU 차이(LiteRaceSegNet - SegFormer-03): {diff:+.4f}")

  if not notes:
    return "LiteRaceSegNet과 SegFormer-03 두 행이 같은 device 안에 있어야 trade-off 문장을 자동 생성할 수 있습니다."
  return "\n".join(notes)
...
def make_comparison_md(rows: List[Dict[str, str]]) -> str:
  ...
  lines.extend([
    ...
    "## 발표용 결론 문장",
    "",
    "LiteRaceSegNet은 SegFormer-03과 같은 validation layout에서 비교되었고, 표는 segmentation 품질뿐 아니라 모델 크기, device별 latency, throughput, GPU memory까지 함께 보여준다. CPU 결과는 GPU가 없는 현장형 배포 가능성을 보여주고, GPU 결과는 AWS 같은 가속 환경에서의 추론 확장성을 보여준다. LiteRaceSegNet이 더 작은 모델 크기와 낮은 latency를 보이면서 mIoU/Damage IoU를 실사용 가능한 수준으로 유지한다면, 경량 도로 손상 segmentation 서비스에 더 적합한 trade-off를 제공한다고 말할 수 있다.",
  ])
  return "\n".join(lines) + "\n"
...
def checkpoint_manifest(literace_ckpt: Path, segformer_ckpt: Path) -> str:
  rows = [
    ...
    ("SegFormer-03", "Transformer", segformer_ckpt),
  ]
...
def make_summary_md(args, compare_rows, service_rows, copies: Dict[str, int]) -> str:
  ...
  checklist = [
    ...
    ("SegFormer-03 best.pth", segformer_ckpt.exists()),
  ...
  lines = [
    ...
    "",
    "## 최종 발표 한 문장",
    "본 프로젝트는 도로 손상 segmentation을 위해 직접 설계한 경량 CNN 기반 LiteRaceSegNet과 Transformer baseline인 SegFormer-03을 동일 데이터셋에서 비교하고, CPU 배포 가능성과 GPU 가속 성능까지 함께 분석한 캡스톤 프로젝트입니다.",
    "",
    "## 자동 점검표",
    "",
    "| 항목 | 상태 |",
  ...
  lines.extend([
    ...
    "- SegFormer-03은 SegFormer-03 전용 checkpoint만 사용합니다.",
    "- 두 모델은 비교표에서만 만납니다.",
    "- LiteRace service summary는 LiteRaceSegNet 결과만 설명합니다.",
    "",
    "## 결과 폴더 구성",
    "- `01_checkpoints/checkpoint_manifest.md`: checkpoint 존재 여부와 크기",
    "- `02_metrics_and_compare/`: CNN vs Transformer 비교 CSV/JSON. CPU/GPU latency, throughput, 모델 크기, GPU memory, mIoU를 같이 확인합니다.",
    "- `03_literace_overlays/`: LiteRaceSegNet service overlay/card/mask",
    "- `04_segformer_overlays/`: SegFormer-03 inference overlay/mask",
...
def main():
  ...
  parser.add_argument("--segformer_infer_dir", default="seg/runs/segformer_03_infer_after_train")
  parser.add_argument("--literace_ckpt", default="seg/runs/literace_boundary_degradation/best.pth")
  parser.add_argument("--segformer_ckpt", default="seg/transformer_03/checkpoints/segformer_03_best.pth")
...
Core rule:
- LiteRaceSegNet and SegFormer-03 are trained separately.
