"""SegFormer-03 adapter for the capstone comparison pipeline.
...
_VARIANT_CONFIGS = {
  ...
  "segformer_03": _B3_CONFIG,
  "03": _B3_CONFIG,
}


class SegFormer03(nn.Module):
  ...
  def __init__(
    ...
    try:
      ...
      raise RuntimeError(
        "SegFormer-03을 사용하려면 transformers 패키지가 필요합니다. "
    ...
    if self.variant not in _VARIANT_CONFIGS:
      raise ValueError(f"Unsupported SegFormer-03 variant: {variant}")
