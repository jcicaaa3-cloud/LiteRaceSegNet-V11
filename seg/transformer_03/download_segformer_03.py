...
Manual usage:
  python seg/transformer_03/download_segformer_03.py \
    --model-id nvidia/segformer-b3-finetuned-ade-512-512 \
    --outdir seg/transformer_03/hf_pretrained/segformer_b3_ade \
    --write-config
"""
...
def main() -> int:
  parser = argparse.ArgumentParser(description="Download local SegFormer-03 HuggingFace baseline")
  parser.add_argument(
    "--model-id",
  ...
  parser.add_argument(
    "--outdir",
    default="seg/transformer_03/hf_pretrained/segformer_b3_ade",
    help="Local output folder inside the project",
  )
  parser.add_argument(
    "--write-config",
    action="store_true",
    help="Also write seg/config/pothole_binary_segformer_03_hf.yaml when missing",
  )
  ...
  manifest = {
    ...
    "recommended_config": "seg/config/pothole_binary_segformer_03_hf.yaml",
  }
  local_folder_for_yaml = manifest["local_folder"].replace("\\", "/")
  with open(outdir / "CAPSTONE_SEGFORMER_03_MANIFEST.json", "w", encoding="utf-8") as f:
    json.dump(manifest, f, ensure_ascii=False, indent=2)

  if args.write_config:
    cfg_path = root / "seg" / "config" / "pothole_binary_segformer_03_hf.yaml"
    if cfg_path.exists():
...
device: cpu
num_workers: 0
save_dir: runs/segformer_03_hf_pothole
...
"""
  ...
  print("[OK] SegFormer-03 HuggingFace baseline prepared.")
