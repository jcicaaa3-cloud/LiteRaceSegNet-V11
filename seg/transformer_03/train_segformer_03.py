"""Fine-tune SegFormer-03/B3 on a binary pothole segmentation dataset.
...
Recommended Windows usage:
  03_TRAIN_SEGFORMER_03_POTHOLE.bat
...
def save_hf_folder_if_possible(model: nn.Module, outdir: Path) -> None:
  # The project wrapper is SegFormer03, and the inner HF module lives at .model.
...
def main() -> int:
  parser = argparse.ArgumentParser(description="Fine-tune HuggingFace SegFormer-03/B3 for binary pothole segmentation")
  parser.add_argument("--config", default="seg/config/pothole_binary_segformer_03_train.yaml")
  ...
  save_dir = resolve_project_path(cfg.get("save_dir", "seg/runs/segformer_03_hf_pothole"))
  ckpt_dir = save_dir / "checkpoints"
  make_dir(str(ckpt_dir))
  transformer_ckpt_dir = PROJECT_ROOT / "seg" / "transformer_03" / "checkpoints"
  transformer_ckpt_dir.mkdir(parents=True, exist_ok=True)
  hf_finetuned_dir = PROJECT_ROOT / "seg" / "transformer_03" / "hf_finetuned" / "pothole_binary"
  ...
  with open(log_path, "w", newline="", encoding="utf-8-sig") as f:
    ...
    for epoch in range(1, epochs + 1):
      ...
      # Always save last.
      save_state(str(ckpt_dir / "segformer_03_last.pth"), model, optimizer, None, scaler, epoch, best_miou, cfg)

      if improved:
        save_state(str(ckpt_dir / "segformer_03_best.pth"), model, optimizer, None, scaler, epoch, best_miou, cfg)
        save_state(str(transformer_ckpt_dir / "segformer_03_best.pth"), model, optimizer, None, scaler, epoch, best_miou, cfg)
        save_hf_folder_if_possible(model, hf_finetuned_dir)
        print(f"[SAVE] best project checkpoint: {transformer_ckpt_dir / 'segformer_03_best.pth'}")
  ...
  print("\n[DONE] SegFormer-03 fine-tuning finished.")
  print(f"- best mIoU: {best_miou:.4f}")
  print(f"- log:    {log_path}")
  print(f"- checkpoint for compare/infer: {transformer_ckpt_dir / 'segformer_03_best.pth'}")
  print("\n[NEXT]")
  print("1) run 04_COMPARE_AFTER_SEGFORMER_03_TRAIN.bat")
  print("2) run 05_INFER_SEGFORMER_03_AFTER_TRAIN.bat")
  return 0
