...
if not exist "seg\transformer_03\checkpoints\segformer_03_best.pth" (
  echo [ERROR] SegFormer-03 checkpoint missing.
  echo Run 03B_TRAIN_SEGFORMER_03_ONLY.bat or 03_TRAIN_BOTH_SEPARATE.bat first.
  pause
  exit /b 1
)
...
echo.
echo [B] SegFormer-03 inference only - separated comparison path
...
python seg\tools\build_final_evidence_package.py ^
 ...
 --segformer_infer_dir seg\runs\segformer_03_infer_after_train
if errorlevel 1 goto failed
