...
python seg\transformer_03\download_segformer_03.py --model-id nvidia/segformer-b3-finetuned-ade-512-512 --outdir seg\transformer_03\hf_pretrained\segformer_b3_ade --write-config
if errorlevel 1 (
  echo [ERROR] SegFormer-03 download/setup failed.
if not defined NO_PAUSE pause
  exit /b 1
)

echo.
echo [OK] SegFormer-03 HuggingFace baseline is ready.
echo Local model folder:
echo  seg\transformer_03\hf_pretrained\segformer_b3_ade
echo.
